  ...
  static async importCookies(cookies) {
    ...
    for (const cookie of cookies) {
      try {
        ...
        // Map sameSite values to Chrome-compatible values
        // Chrome accepts: "unspecified", "no_restriction", "lax", or "strict"
...
class UI {
  ...
  static showImportOptions() {
    const dialog = document.getElementById('import-options');
    dialog.className = 'modal';
    ...
    return new Promise((resolve) => {
      ...
      document.getElementById('chooseFileBtn').onclick = handleChooseFile;
      document.getElementById('pasteDataBtn').onclick = handlePasteData;
    });
  }

  static async showPasteDialog() {
    return new Promise((resolve) => {
      ...
      const cleanup = () => {
        ...
        document.getElementById('confirmPaste').removeEventListener('click', handleConfirm);
        document.getElementById('cancelPaste').removeEventListener('click', handleCancel);
      };

      textarea.addEventListener('keydown', handleKeydown);
      document.getElementById('confirmPaste').addEventListener('click', handleConfirm);
      document.getElementById('cancelPaste').addEventListener('click', handleCancel);
    });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  importBtn.addEventListener('click', async () => {
    try {
      const importMethod = await UI.showImportOptions();
      if (!importMethod) return;

      let importData;
      if (importMethod === 'file') {
        importData = await UI.getFileContent();
      } else {
        importData = await UI.showPasteDialog();
      }
